...
//        ]);
        ...
        if($request->has('in_file')){
            ...
            $new_file_name = $file_temp->getClientOriginalExtension();
            $cv_file = $file_temp->move(public_path('cv_upload'),$new_file_name);
            // var_dump($new_file_name);die;
        }
        $validator = \Validator::make($request->all(), $rules,$custom_message);
        if($request->has('in_file')){
            $file_temp2 = $request->in_file;
            $file_data = $file_temp2->getClientOriginalName();
            $file_name_temp = $request->get('file_temp');
            if($file_data != $file_name_temp ){
                $new_file_name = '';
                if($request->has('in_file')){
                    $file_temp = $request->in_file;
                    $new_file_name = $request->get('in_cvno').'_'.$request->get('in_lastname').'_'.$request->get('in_firstname').'.'.$file_temp->getClientOriginalExtension();
                    $cv_file = $file_temp->move(public_path('cv_upload'),$new_file_name);
                    // var_dump($new_file_name);die;
                }
            }
            ...
            if($new_file_name != ''){
                unlink("cv_upload/".$new_file_name);
            }
        }
